...
demoApp.controller('ProCtrl', function ($scope, $http) {
    ...
    $scope.saveProduct = function () {
        $http({
            method: 'POST',
            url: 'http://localhost:8084/AngularDemoPractices/rest/api/products/add',
    ...
    $scope.deleteProduct = function () {
        $http({
            method: 'DELETE',
            url: 'http://localhost:8084/AngularDemoPractices/rest/api/products/' + $scope.clickedProduct.code
        }).then(
                function (response) {
...
var response = $http.get('http://localhost:8080/api/products/productlist');
	...
	$scope.add_cart = function(product){ //set a function name add_cart
					if(product){ //check if the product is already declared within the function
						...
						$scope.carts.push({pid: product.pid, pname: product.pname, pprice: product.pprice,pimage:product.pimage}); //pushes the chosen product into a new variable called carts when the add to cart button is clicked
					}	
				};
				...
				$scope.setTotals = function(cart){ //set a function name setTotals 
					if(cart){ //check if cart is already set in the function
						$scope.total += (cart.pprice); //sum the total value of each product
					}
				};
				...
				$scope.remove_cart = function(cart){ //set a function called remove_cart
					if(cart){ //checked if the cart has a value
						...
						$scope.total -= (cart.pprice); //deduct the price of the product  simultaneously when deleted
					}
				};
...
//        $http({
//            mehtod: "GET",
//            url: '/rest/api/products/productlist'
//        }).then(
//                function (response) {
//                    $scope.products = response.data;
//                },
